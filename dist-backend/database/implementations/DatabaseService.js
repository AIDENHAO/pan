...
export class DatabaseService {
     ...
     * 获取完整的人物信息
     */
    async getCompleteCharacterInfo(characterId) {
        try {
     ...
     * 分页获取人物列表
     */
    async getCharacterList(page = 1, pageSize = 20, options) {
        try {
     ...
     * 搜索人物
     */
    async searchCharacters(searchTerm, searchType = 'name') {
        try {
     ...
     * 更新人物修炼值
     */
    async updateCultivation(characterId, cultivationValue) {
        try {
     ...
     * 添加物品到人物背包
     */
    async addItemToCharacter(characterId, itemId, count = 1, level = 1) {
        try {
            // 检查是否已有该物品
     ...
     * 装备物品
     */
    async equipItem(characterId, itemInstanceId, slotPosition) {
        const transaction = dalFactory.createTransaction();
        try {
            return await transaction.execute(async () => {
                // 检查物品是否属于该人物
