...
if (process.env.NODE_ENV === 'development') {
  ...
  try {
    // 测试路由导入
    import('../routes/databaseRoutes.js').then((databaseRoutes) => {
      console.log('✅ 数据库路由导入成功');
      console.log('✅ 数据库路由类型:', typeof databaseRoutes.default);
    }).catch((error) => {
      console.error('❌ 数据库路由导入失败:', error);
    });
  } catch (error) {
