...
function generateTestReport(testResults: any[], totalDuration: number): string {
  ...
  // 构建通过的测试列表
  const passedTestsList = testResults
  ...
  // 计算各类测试的成功率
  const serverTests = testResults.filter((r: any) => r.name.includes('服务器连接'));
  const serverSuccessRate = serverTests.length > 0 ? 
    (serverTests.filter(r => r.passed).length / serverTests.length * 100).toFixed(2) : '0.00';

  const characterTests = testResults.filter((r: any) => r.name.includes('角色'));
  const characterSuccessRate = characterTests.length > 0 ? 
    (characterTests.filter(r => r.passed).length / characterTests.length * 100).toFixed(2) : '0.00';

  const dataTests = testResults.filter((r: any) => r.name.includes('数据查询'));
  const dataSuccessRate = dataTests.length > 0 ? 
    (dataTests.filter(r => r.passed).length / dataTests.length * 100).toFixed(2) : '0.00';

  const errorTests = testResults.filter((r: any) => r.name.includes('错误') || r.name.includes('验证'));
  const errorSuccessRate = errorTests.length > 0 ? 
    (errorTests.filter(r => r.passed).length / errorTests.length * 100).toFixed(2) : '0.00';
...
### 服务器连接
- 测试数量: ${serverTests.length}
- 通过数量: ${serverTests.filter(r => r.passed).length}
- 成功率: ${serverSuccessRate}%

### 角色管理API
- 测试数量: ${characterTests.length}
- 通过数量: ${characterTests.filter(r => r.passed).length}
- 成功率: ${characterSuccessRate}%

### 静态数据API
- 测试数量: ${dataTests.length}
- 通过数量: ${dataTests.filter(r => r.passed).length}
- 成功率: ${dataSuccessRate}%

### 错误处理和验证
- 测试数量: ${errorTests.length}
- 通过数量: ${errorTests.filter(r => r.passed).length}
- 成功率: ${errorSuccessRate}%
...
${failedTests > 0 ? '### 发现的问题\n\n' + failedTestsList.replace(/\d+\. \*\*(.+?)\*\*/g, '#### $1失败\n\n**错误现象**: 测试失败\n\n**可能原因**:\n1. API服务器未启动或端点不存在\n2. 数据库连接问题\n3. 路由配置错误\n4. 控制器方法未实现\n\n**建议解决方案**:\n1. 检查API服务器是否正常运行\n2. 验证路由配置是否正确\n3. 确认控制器方法是否已实现\n4. 检查数据库连接状态') : '### 🎉 所有测试通过\n\n所有API端点都正常工作，没有发现问题。'}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const reportPath = path.join(currentDir, 'api-test-report.md');
