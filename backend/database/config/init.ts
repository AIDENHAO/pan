...
export class DatabaseInitializer {
  ...
  public async isInitialized(): Promise<boolean> {
    try {
      ...
      const result = await dbManager.get(
        "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'character_base_info'"
      );
  ...
  public async reset(): Promise<void> {
    try {
      ...
      // 获取所有表名
      const tables = await dbManager.all<{name: string}>(
